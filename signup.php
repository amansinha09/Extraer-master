...
<body>
 <div class="container">
<div class="login">
  <header class="login-header"><span class="text">SIGNUP</span><span class="loader"></span></header>
  <form action="signup.php" method="post" class="login-form">
    ...
    <button class="login-btn" name="add" type="submit">Sign Up</button>
  </form>
</div>
</div> 
...
<?php 
	if(isset($_POST['add']))
	{
        ...
        $find_query="SELECT * FROM `users` WHERE username LIKE '$username' AND password LIKE '$password'";
        $run_find_query=mysqli_query($connection,$query);
        foreach ($run_find_query as $ans) {
          $id=$ans['id'];
        }
        $_SESSION['id']=$id;
